...
function SquishyModelFactory.build(def): Model
	...
	if meshTemplate then
		-- RenderFidelity (Precise, so distant friends don't drop to a crude box
		-- "outline") is baked onto the stored MeshBodies templates at edit time
		-- — it CANNOT be written from a runtime script (needs Plugin capability),
		-- and Clone() inherits it from the template anyway.
