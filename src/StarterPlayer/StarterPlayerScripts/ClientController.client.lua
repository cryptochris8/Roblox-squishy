...
StreamerMode.init()
GardenUI.mount(playerGui, function(bedId, seedId)
	Remotes.get(Remotes.PlantSeed):FireServer(bedId, seedId)
end, function(bedId)
	Remotes.get(Remotes.HarvestPlant):FireServer(bedId)
end)
...
Remotes.get(Remotes.OpenGardenUI).OnClientEvent:Connect(function()
	GardenUI.open()
end)
